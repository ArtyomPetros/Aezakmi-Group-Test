...
struct EditorView: View {
    ...
    @State var infoText: String = "Нажмите, чтобы добавить текст"
    ...
    var body: some View {
                ...
                VStack(spacing: 0) {
                    HStack {
                        ...
                        Picker("Режим", selection: $mode) {
                            Text("Рисунок").tag(DrawingMode.draw)
                            Text("Текст").tag(DrawingMode.text)
                        }
                        ...
                        if (mode == .draw) {
                            ...
                            ZStack(alignment: .center) {
                                ...
                                if (selectedTextView != nil) {
                                    ...
                                    // Text hint
                                    Text(!isTextVisible ? "Активирован режим скрытия текста" : infoText).fontWeight(.bold).foregroundColor(.light)
                                        .onTapGesture {
                                            if isTextVisible {
                                                addText()
                                            }
        ...
        .alert(isPresented: self.$isDismissAlertPresented) {
            ...
            Alert(
                ...
                message: Text("Изменения не будут сохранены"),
                primaryButton: .cancel(Text("Отмена")),
                secondaryButton: .destructive(Text("Покинуть"), action: {
                    close()
                })
            )
        }
    ...
    func addText() {
        ...
        canvasController?.showTextAlert(title: "Добавить текст", text: nil, actionTitle: "Добавть") { text in
            addTextView(text)
        }
    }
    ...
    /// Save drawings and texts with selected  image to user's media library
    func export() {
        withAnimation {
            isProcesing = true
        }
    ...
    func modeChanged(_ selected: DrawingMode) {
        ...
        if (selected == .text) {
            ...
            let labels: [UIView] = canvasController?.view.subviews.filter { $0 is UILabel } ?? []
            infoText = labels.isEmpty ? "Нажмите, чтобы добавить текст" : "Нажмите на любой текст, чтобы настроить его"
    ...
    func selectionChanged(_ view: UIView?) {
        ...
        infoText = "Нажмите на любой текст, чтобы настроить его"
    }
}
